...
// Basit mock detection fonksiyonu
function mockDetectPersonalData(text: string) {
  const mockPatterns = {
  ...
  Object.entries(mockPatterns).forEach(([type, pattern]) => {
    let match;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (file.type === 'text/plain' || file.name.endsWith('.txt') || file.name.endsWith('.csv')) {
      ...
      text = `[${file.name} dosyasından çıkarılan test metni]
...
- Kredi Kartı: 4532 1234 5678 9012`;
      console.log('Mock text created');
    }
    
    // Kişisel verileri tespit et
    console.log('Starting detection...');
    const detectedData = mockDetectPersonalData(text);
